...
export default function AdminLoginPage() {
  ...
  useEffect(() => {
    ...
    // Using warmer colors to match Admin Red/Orange theme
    const kiteColors = [
      "text-red-600",
      "text-orange-600",
      "text-yellow-600",
    ...
    const newKites = Array.from({ length: 25 }).map((_, i) => ({
      ...
      delay: Math.random() * -35, // Negative delay for immediate visibility
    ...
    const newLanterns = Array.from({ length: 18 }).map((_, i) => ({
      ...
      color: "text-red-400", // Reddish tint for Admin theme
  ...
  useEffect(() => {
    ...
    const loadTurnstile = async () => {
      if (window.turnstile) {
        try {
          turnstileWidgetRef.current = window.turnstile.render("#turnstile-container-admin", {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("[v0] Submitting login with CAPTCHA token")
      ...
      const data = await response.json()
      console.log("[v0] Login response:", { success: data.success })

      if (data.success) {
        localStorage.setItem("adminAuth", data.token)
        localStorage.setItem("adminToken", data.token)  // Also set adminToken for compatibility
        localStorage.setItem("adminData", JSON.stringify(data.admin))
        localStorage.removeItem("adminLoginAttempts")
        localStorage.removeItem("adminLoginLockout")
        setFailedAttempts(0)
        setLockoutTime(0)
        console.log("[v0] Admin login successful, tokens saved")
  ...
  return (
    <div className="min-h-screen relative w-full overflow-hidden bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 dark:from-gray-950 dark:via-red-950 dark:to-orange-950 flex items-center justify-center p-4">
            ...
            {lanterns.map((lantern) => (
              ...
              >
                <div className="relative w-full h-full">
                  <div className="absolute inset-0 bg-red-500 blur-[25px] opacity-30 rounded-full scale-125 animate-pulse"></div>
        ...
        <Card className="shadow-2xl border-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm">
          <CardHeader className="text-center space-y-4 pb-8">
            ...
            <div className="space-y-2">
              <CardTitle className="text-3xl font-bold text-red-600 dark:text-red-400">Admin Portal</CardTitle>
              <CardDescription className="text-base text-muted-foreground">
                Sign in to manage the placement system
              </CardDescription>
            </div>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-11 bg-background/50 border-2 focus:border-red-500 transition-colors"
                  required
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    className="h-11 bg-background/50 border-2 focus:border-red-500 transition-colors pr-10"
                    required
                    disabled={loading || lockoutTime > 0}
                  />
                  <Button
              ...
              <div className="space-y-2">
                <Label className="text-sm font-medium">Verify you are Alien</Label>
                <div id="turnstile-container-admin" ref={turnstileContainerRef} className="flex justify-center" />
              </div>
              ...
              <Button
                type="submit"
                className="w-full h-11 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-medium shadow-lg hover:shadow-xl transition-all duration-200"
                disabled={loading || lockoutTime > 0}
              >
                {loading ? (
      ...
      <ForgotPasswordDialog 
        ...
        userType="admin"
        identityFieldLabel="Username"
        identityFieldPlaceholder="Enter your username"
      />
