...
export default function TechnicalTeamLogin() {
  ...
  useEffect(() => {
    ...
    // Using Green/Teal/Emerald shades to match Technical Theme
    const kiteColors = [
      "text-green-600",
      "text-emerald-600",
      "text-teal-600",
    ...
    // Keeping warm orange for realism, but maybe a few teal ones for "tech" vibe
    const newLanterns = Array.from({ length: 18 }).map((_, i) => ({
      ...
      color: "text-orange-400", // Keep standard warm lantern color
  ...
  useEffect(() => {
    ...
    const storedAttempts = localStorage.getItem("technicalLoginAttempts")
    const storedLockout = localStorage.getItem("technicalLoginLockout")
  ...
  useEffect(() => {
    ...
    const loadTurnstile = async () => {
      if (window.turnstile) {
        try {
          turnstileWidgetRef.current = window.turnstile.render("#turnstile-container-technical", {
  ...
  useEffect(() => {
    if (lockoutTime > 0) {
      ...
      setFailedAttempts(0)
      localStorage.removeItem("technicalLoginAttempts")
      localStorage.removeItem("technicalLoginLockout")
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/technical/login", {
        method: "POST",
      ...
      if (data.success) {
        localStorage.setItem("technicalTeamAuth", data.token)
        localStorage.setItem("technicalTeamData", JSON.stringify(data.user))
        localStorage.removeItem("technicalLoginAttempts")
        localStorage.removeItem("technicalLoginLockout")
        setFailedAttempts(0)
        setLockoutTime(0)
        ...
        localStorage.setItem("technicalLoginAttempts", newAttempts.toString())

        if (newAttempts >= 5) {
          const lockoutEnd = Date.now() + 10 * 60 * 1000
          localStorage.setItem("technicalLoginLockout", lockoutEnd.toString())
  ...
  return (
    <div className="min-h-screen relative w-full overflow-hidden bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-gray-900 dark:via-emerald-900 dark:to-teal-900 flex items-center justify-center p-4">
        ...
        <Card className="shadow-2xl border-0 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
          ...
          <CardContent className="space-y-6">
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-11 bg-background/50 border-2 focus:border-emerald-500 transition-colors"
                  required
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    className="h-11 bg-background/50 border-2 focus:border-emerald-500 transition-colors pr-10"
                    required
                    disabled={loading || lockoutTime > 0}
                  />
                  <Button
              ...
              <div className="space-y-2">
                ...
                <div id="turnstile-container-technical" ref={turnstileContainerRef} className="flex justify-center" />
              </div>
              ...
              <Button
                type="submit"
                className="w-full h-11 bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 hover:from-green-700 hover:via-emerald-700 hover:to-teal-700 text-white font-medium shadow-lg hover:shadow-xl transition-all duration-200"
                disabled={loading || lockoutTime > 0}
              >
                {loading ? (
            ...
            <div className="text-center pt-4 border-t">
              <p className="text-sm text-muted-foreground">Technical Team personnel only</p>
            </div>
          </CardContent>
        </Card>
      </div>
